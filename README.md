...
本库命名空间已统一在 `Jory.*` 根命名空间下，引用时请注意：

| 命名空间 | 包含模块 |
|----------|----------|
| `Jory.Common` | 文件 / 目录、字节转换、MVVM 基类、WPF 转换器、INI/XML 读写、枚举扩展、通用函数等全部公共模块 |
| `Jory.IOC` | IOC 容器（`Container`、`ContainerExtension`、`DependencyDescriptor`、特性与接口） |
| `Jory.Log` | `NLogger` 日志封装 |

---

## 模块用法

### 1. IOC 依赖注入容器（Jory.IOC）
